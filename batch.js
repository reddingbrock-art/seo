...
function pageVars(row) {
  ...
  const faqBank = {
    ...
    "lead-followup": [
      ...
      "Does the follow-up only go to people who reached out to us first?",
...
- Before <!-- SECTION 2: INTRO -->, output a single HTML comment in this exact format:
  <!-- HERO_SUB: [one sharp sentence — outcome-focused, present-tense pain, max 12 words, no fluff. Good examples: "Your next job went to whoever texted back first." / "Every unanswered call in ${v.city} is money in your competitor's pocket." / "Most ${v.vertical} owners in ${v.city} are losing jobs before they even know they had one."] -->
- Start your sections with exactly: <!-- SECTION 2: INTRO -->
...
BANNED CONCEPTS — never mention any of these, ever:
Field-Built Systems ONLY offers: done-for-you CRM setup, AI chat agent, AI voice receptionist, automated lead follow-up sequences (inbound leads only), automated Google review requests, and GoHighLevel configuration. Nothing else.
NEVER mention or imply FBS offers any of the following — these are not services FBS provides:
- Voicemail drops, ringless voicemail, or any outbound calling tactic
- Seasonal reactivation campaigns or any campaign management
- Email marketing, newsletters, or drip campaigns to cold contacts
- Social media management, posting, or advertising
- SEO services or content marketing
- PPC, Google Ads, Facebook Ads, or any paid advertising
- Website design or development (beyond what's included in the done-for-you system)
- Reputation monitoring beyond Google reviews
- Field service dispatching or scheduling software
- Payroll, invoicing, or accounting integrations
- Any outbound prospecting of any kind — FBS only handles inbound lead response and follow-up
- Any service not explicitly listed above as what FBS offers
If you are tempted to mention any of these, do not. Stick strictly to what FBS actually installs.
...
function buildPage(v, content) {
  ...
  const service = {
    ...
    "url": `https://seo.field-built.com/${v.slug}`
  };
  ...
  // Extract dedicated hero subhead written by Claude
  const heroSubMatch = content.match(/<!-- HERO_SUB: (.*?) -->/);
  const heroSub = heroSubMatch
    ? heroSubMatch[1].trim()
...
<head>
  ...
  <link rel="canonical" href="https://seo.field-built.com/${v.slug}">
  <meta property="og:title" content="${v.h1} | Field-Built Systems">
  <meta property="og:description" content="${v.metaDesc}">
  <meta property="og:url" content="https://seo.field-built.com/${v.slug}">
